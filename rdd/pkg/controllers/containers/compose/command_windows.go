...
func (c *concreteCommandExecutor) spawn() error {
	...
	if pathErr, ok := errors.AsType[*os.PathError](err); ok && pathErr.Op == "chdir" {
		...
		if err == nil {
			...
			c.Cmd.Process.WithHandle(func(handle uintptr) {
			...
			if err != nil {
				return fmt.Errorf("failed ensure temporary directory will be deleted: %w", err)
			}
			go func() {
				defer windows.CloseHandle(hProc)
				defer os.RemoveAll(temporaryWorkingDir)
